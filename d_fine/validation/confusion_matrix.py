...
class ConfusionMatrix:
    ...
    def update(self, gt_label: int | None, pred_label: int | None):
        g_idx = self.class_to_idx.get(gt_label, self.n_classes) if gt_label is not None else self.n_classes
        p_idx = self.class_to_idx.get(pred_label, self.n_classes) if pred_label is not None else self.n_classes
        self.matrix[g_idx, p_idx] += 1

    def plot(self, save_path: Path, label_to_name: dict[int, str]):
        class_labels = [label_to_name.get(cls_id, str(cls_id)) for cls_id in sorted(self.class_to_idx.keys())] + ["background"]
        ...
        for i, j in np.ndindex(self.matrix.shape):
            plt.text(j, i, format(self.matrix[i, j], "d"), ha="center", va="center",
