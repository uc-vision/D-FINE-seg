...
import numpy as np
import torch
from lib_detection.annotation import find_instance_overlaps
...
class SparseMaskAP:
  ...
  def _compute_class_ap(
    self, label: int, gt: list[ImageResult], preds: list[ImageResult], all_ious: list[np.ndarray]
  ) -> np.ndarray | None:
    ...
    if not class_preds:
      return np.zeros(len(self.thresholds))

    num_thresholds, num_preds = len(self.thresholds), len(class_preds)
    img_gt_matched = [
      np.zeros((num_thresholds, len(idx)), dtype=bool) for idx in gt_indices_per_img
    ]
    ...
    for p_idx, (_, img_idx, p_mask_idx) in enumerate(class_preds):
      ...
      is_match, best_gt_local_idx = find_best_matches(
        local_ious, img_gt_matched[img_idx], self.thresholds
      )
    ...
    return np.array(
      [
        compute_ap_from_counts(tps[t_idx], fps[t_idx], num_ground_truth)
