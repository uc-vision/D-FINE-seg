...
from d_fine.trainer import Trainer
...
def main(project_name: str, base_path: Path, exp_name: str | None) -> None:
  ...
  # Trainer instance for postprocessing
  ...
  def validate(compiled_model: ov.CompiledModel, validation_loader: DataLoader) -> float:
    ...
    all_preds: list[ImageResult] = []
    all_gt: list[ImageResult] = []
    ...
    for inputs, targets, _ in validation_loader:
      ...
      preds = trainer.preds_postprocess(inputs, outputs, orig_sizes)
      gt = trainer.gt_postprocess(inputs, targets, orig_sizes)
    ...
    validator = Validator(all_gt, all_preds, config=val_cfg)
  ...
  ov.save_model(quantized_model, int8_xml_path, compress_to_fp16=False)
