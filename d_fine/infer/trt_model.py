...
class TRT_model(InferenceModel):
    ...
    def from_train_config(cls, train_config: TrainConfig) -> TRT_model:
        ...
        model_path = str(train_config.paths.path_to_save / "model.engine")
        ...
        return cls(model_config, model_path)
    ...
    def _preprocess(self, img: NDArray, stride: int = 32) -> torch.Tensor:
        if not self.keep_ratio:
            ...
            img = infer_utils.letterbox(img, (self.input_size[0], self.input_size[1]), stride=stride, auto=False)[0]
    ...
    def _prepare_inputs(self, inputs):
        ...
        if isinstance(inputs, np.ndarray) and inputs.ndim == 3:
            ...
            processed_inputs = torch.zeros((inputs.shape[0], self.channels, *self.input_size), dtype=torch.from_numpy(np.zeros(0, dtype=self.np_dtype)).dtype)
