...
class Torch_model(InferenceModel):
    ...
    def from_train_config(
        ...
        return cls(model_config, model_path, use_nms=use_nms, device=device)
    ...
    def _prepare_input(self, img: NDArray[np.uint8]) -> tuple[torch.Tensor, tuple[int, int], tuple[int, int]]:
    ...
    def predict_batch(self, imgs: NDArray[np.uint8]) -> list[ImageResult]:
        ...
        for i in range(batch_size):
            tensor, orig_size = infer_utils.preprocess(
                imgs[i], self.input_size, self.keep_ratio, self.rect, dtype=self.np_dtype
            )
