...
def scale_boxes_ratio_kept(boxes: np.ndarray, proc_size: tuple[float, float], orig_size: tuple[float, float]) -> np.ndarray:
...
def process_boxes(boxes: torch.Tensor, processed_size: tuple[int, int], orig_sizes: torch.Tensor, keep_ratio: bool, device: torch.device) -> torch.Tensor:
    """
...
def process_masks(masks: torch.Tensor, bboxes: torch.Tensor, labels: torch.Tensor, scores: torch.Tensor, processed_size: tuple[int, int], orig_sizes: torch.Tensor, keep_ratio: bool = True, conf_thresh: float = 0.0) -> list[list[InstanceMask]]:
    """
    ...
    for i in range(B):
        ...
        gain = min(Hp / h0, Wp / w0) if keep_ratio else 1.0
        padw = (Wp - w0 * gain) / 2 if keep_ratio else 0.0
        padh = (Hp - h0 * gain) / 2 if keep_ratio else 0.0
        ...
        for j in range(K):
            ...
            if m_crop.numel() == 0: continue
            
            m_final = F.interpolate(m_crop[None, None], size=(oy2-oy1, ox2-ox1), mode="bilinear", align_corners=False)[0, 0]
            m_bool = m_final >= conf_thresh
            
            img_instances.append(InstanceMask(
                mask=m_bool.cpu(),
                label=label,
                offset=(ox1, oy1),
                score=score
...
def postprocess_predictions(
    ...
    abs_boxes = process_boxes(boxes, processed_size, orig_sizes, config.keep_ratio, logits.device)
    ...
    if config.use_focal_loss:
        ...
        topk_labels = topk_idx % config.n_outputs
